...
local function emit_before_change(op, metadata)
    ...
    -- If no callbacks are registered, proceed by default
    if #before_change_callbacks == 0 then
        return true
    end
    ...
    return not should_abort -- Return true if the operation should proceed, false otherwise
end
...
M.create_worktree = function(path, branch, upstream)
    ...
    local should_proceed = emit_before_change(Enum.Operations.Create, { path = path, prev_path = vim.loop.cwd() })
    if not should_proceed then
        print("Worktree change canceled by before_change hook.")
        return
    end
...
M.switch_worktree = function(path)
    ...
    local should_proceed = emit_before_change(Enum.Operations.Switch, { path = path, prev_path = vim.loop.cwd() })
    if not should_proceed then
        print("Worktree change canceled by before_change hook.")
        return
    end
...
M.delete_worktree = function(path, force, opts)
    ...
    local should_proceed = emit_before_change(Enum.Operations.Delete, { path = path, prev_path = vim.loop.cwd() })
    if not should_proceed then
        print("Worktree change canceled by before_change hook.")
        return
    end
